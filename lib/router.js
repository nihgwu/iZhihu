...
router.get('/undefined', function (req, res, next) {
  res.sendFile(path.join(__dirname, '../public/img/noimage.jpg'));
});
...
router.get('/date', function (req, res, next) {
  ...
  var endDate = moment().subtract(30, 'days').format('YYYYMMDD');
  var news = [];
...
router.get('/date/before/:date', function (req, res, next) {
  var startDate = moment(req.params.date, 'YYYYMMDD').subtract(1, 'days').format('YYYYMMDD');
  var endDate = moment(req.params.date, 'YYYYMMDD').subtract(30, 'days').format('YYYYMMDD');
  var news = [];
  db.allDocs({startkey: 'daily-' + startDate, endkey: 'daily-' + endDate, include_docs: true, descending: true}).then(function (data) {
    data.rows.forEach(function (item) {
      var json = item.doc;
      ...
      db.get('daily-' + json.date).then(function (data) {
        for (var i = 0; i < data.news.length; i++) {
          if (data.news[i].id == json.id) {
            ...
            var next = data.news[i + 1] || null;
            if (!pre) {
              var after = moment(json.date, 'YYYYMMDD').add(1, 'days').format('YYYYMMDD');
